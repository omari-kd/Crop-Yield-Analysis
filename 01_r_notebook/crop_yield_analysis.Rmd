...
```{r message=F}
ggplot(data = clean_data, mapping = aes(x = `Rainfall (mm)`, y = `Yield (Q/acre)`)) +
...
```{r message=F}
ggplot(data = clean_data, mapping = aes(x = `Temperature`, y = `Yield (Q/acre)`)) +
  geom_point(color = "darkblue") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
...
```{r}
ggplot(data = clean_data, mapping = aes(x = `Rainfall (mm)`, y = `Yield (Q/acre)`, color = Temperature)) +
  geom_point(size = 3) +
...
```{r}
ggplot(clean_data, aes(x = as.factor(Fertilizer), y = `Yield (Q/acre)`)) +
...
```{r}
ggplot(data = clean_data, mapping = aes(x = Fertilizer, y = `Rainfall (mm)`, color = `Rainfall (mm)`)) + geom_point(size = 3) + 
...
model_full <- lm(`Yield (Q/acre)` ~ Fertilizer + I(Fertilizer^2) + `Rainfall (mm)` + `Nitrogen (N)` + `Phosphorus (P)` + `Potassium (K)`, data = clean_data)
...
ggplotly(ggplot(clean_data, aes(x = Fertilizer, y = `Yield (Q/acre)`)) +
...
rain_mean <- mean(clean_data$`Rainfall (mm)`)
N_mean <- mean(clean_data$`Nitrogen (N)`)
P_mean <- mean(clean_data$`Phosphorus (P)`)
K_mean <- mean(clean_data$`Potassium (K)`)

# Create new data frame for prediction
# Create prediction data frame with correct column names
pred_data <- data.frame(
  Fertilizer = fert_range,
  `Rainfall (mm)` = rain_mean,
  `Nitrogen (N)` = N_mean,
  `Phosphorus (P)` = P_mean,
  `Potassium (K)` = K_mean
)
...
# Plot
ggplot() +
  geom_point(data = clean_data, aes(x = Fertilizer, y = Yield), alpha = 0.5, color = "gray") +
...
```{r}
model1 <- lm(`Yield (Q/acre)` ~ `Rainfall (mm)` + Temperature + Fertilizer + `Nitrogen (N)` + `Phosphorus (P)` + `Potassium (K)`, data = clean_data)
...
```{r}
model2 <- lm(`Yield (Q/acre)` ~ `Rainfall (mm)` * Fertilizer + Temperature + `Nitrogen (N)` + `Phosphorus (P)` + `Potassium (K)`, data = clean_data)
